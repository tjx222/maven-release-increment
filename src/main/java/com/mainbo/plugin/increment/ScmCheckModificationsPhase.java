...
/**
 * See if there are any local modifications to the files before proceeding with
 * SCM operations and the release.
 * 
...
public class ScmCheckModificationsPhase extends AbstractReleasePhase {
   ...
   */
  private static final List<String> exclusionPatterns = Arrays.asList("**" + File.separator + "pom.xml",
  ...
  public ReleaseResult execute(ReleaseDescriptor releaseDescriptor, ReleaseEnvironment releaseEnvironment,
    ...
    for (MavenProject mp : reactorProjects) {
      try {
        ...
        repository = scmRepositoryConfigurator.getConfiguredRepository(mp.getScm().getConnection(), releaseDescriptor,
            releaseEnvironment.getSettings());

        provider = scmRepositoryConfigurator.getRepositoryProvider(repository);
      } catch (ScmRepositoryException e) {
        throw new ReleaseScmRepositoryException(e.getMessage() + " for URL: " + releaseDescriptor.getScmSourceUrl(),
      ...
      try {
        result = provider.status(repository, new ScmFileSet(mp.getBasedir()));
      ...
      List<ScmFile> mpchangedFiles = result.getChangedFiles();

      if (!mpchangedFiles.isEmpty()) {
        ...
        for (Iterator<ScmFile> i = mpchangedFiles.iterator(); i.hasNext();) {
          ScmFile f = i.next();
      ...
      if (!mconfig.getIgnoreUpdate()) {
        ...
        try {
          ...
          throw new ReleaseExecutionException("An error occurred during the update process: " + e.getMessage(), e);
        }

        if (!upresult.isSuccess()) {
          throw new ReleaseScmCommandException("Unable to update ", upresult);
        }
      }

      changedFiles.addAll(mpchangedFiles);
    }
