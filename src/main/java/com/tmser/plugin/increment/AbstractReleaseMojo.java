...
public abstract class AbstractReleaseMojo extends AbstractMojo {
  ...
  /**
   * Role hint of the {@link org.apache.maven.shared.release.exec.MavenExecutor}
   ...
   */
  @Parameter(alias = "incrementPackageArgs", property = "arguments", defaultValue = " -Dmaven.test.skip=true")
  ...
  protected ReleaseEnvironment getReleaseEnvironment() {
    return new DefaultReleaseEnvironment().setSettings(settings).setJavaHome(javaHome).setMavenHome(mavenHome)
        .setLocalRepositoryDirectory(localRepoDirectory).setMavenExecutorId(mavenExecutorId);
  }
  ...
  protected ReleaseDescriptor createReleaseDescriptor() {
    ReleaseDescriptor descriptor = new MergeReleaseDescriptor();
   ...
   *          The release manager implementation to use, must not be
   *          <code>null</code>.
   */
   ...
   *
   * @param basedir
   *          The build's base directory, must not be <code>null</code>.
   */
   ...
   *
   * @param argument
   *          The argument to add, must not be <code>null</code>.
   */
   ...
   * command line system properties and copies
   * it into the release config object.
   *
   * @param config
   *          The release configuration to merge the system properties into,
   *          must not be <code>null</code>.
   * @param sysPropertiesConfig
   *          The configuration from the system properties to merge in, must not
   *          be
   *          <code>null</code>.
   */
  protected void mergeCommandLineConfig(ReleaseDescriptor config, ReleaseDescriptor sysPropertiesConfig) {
    // If the user specifies versions, these should override the existing
    // versions
